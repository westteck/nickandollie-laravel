...
<section class="mx-auto max-w-6xl px-4 py-8 sm:py-12">
    <div class="mb-6 flex items-center justify-between">
        ...
        <a href="{{ route('admin.contests') }}" class="rounded-md bg-[#8b7355] px-4 py-2 text-sm font-medium text-white hover:bg-[#7a6548]">
            + New Contest
        </a>
    </div>
    ...
    <div class="rounded-lg border bg-white shadow-sm">
        ...
        @if($contests->isEmpty())
            <div class="p-8 text-center text-slate-500">No contests yet.</div>
        @else
                            ...
                            <tr class="hover:bg-slate-50">
                                ...
                                <td class="px-4 py-3">
                                    <span class="rounded-full px-2 py-0.5 text-xs font-medium
                                        ...
                                        @elseif($contest->status === 'draft') bg-slate-100 text-slate-600
                                        @else bg-yellow-100 text-yellow-700 @endif">
                                        {{ ucfirst($contest->status) }}
                                    </span>
                                </td>
                                <td class="px-4 py-3 text-slate-600">{{ $contest->start_date ? \Carbon\Carbon::parse($contest->start_date)->format('M j, Y') : '—' }}</td>
                                <td class="px-4 py-3 text-slate-600">{{ $contest->end_date ? \Carbon\Carbon::parse($contest->end_date)->format('M j, Y') : '—' }}</td>
                                <td class="px-4 py-3 text-slate-600">{{ $contest->prize ?: '—' }}</td>
                                <td class="px-4 py-3">
    ...
    <div class="mt-8 rounded-lg border bg-white shadow-sm">
        ...
        <form method="POST" action="{{ $isEditing ? route('admin.contests.update', $editing->id) : route('admin.contests.store') }}"
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                ...
                <div>
                    <label class="mb-1 block text-sm font-medium text-slate-700">Icon (e.g. fa-trophy)</label>
                ...
                <div>
                    <label class="mb-1 block text-sm font-medium text-slate-700">Prize</label>
            ...
            <div>
                ...
                <textarea name="description" rows="3"
                          class="w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:border-[#8b7355] focus:outline-none focus:ring-1 focus:ring-[#8b7355]">{{ old('description', $isEditing ? $editing->description : '') }}</textarea>
            </div>

            <div>
                <label class="mb-1 block text-sm font-medium text-slate-700">Rules</label>
                <textarea name="rules" rows="3"
                          class="w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:border-[#8b7355] focus:outline-none focus:ring-1 focus:ring-[#8b7355]">{{ old('rules', $isEditing ? $editing->rules : '') }}</textarea>
            </div>
